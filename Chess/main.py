...
class Board(list):
    def __init__(self, board_state=None, color=None, castle_rights=None, *args, **kwargs):
        ...
        if board_state is not None:
            ...
            self.update_board(board_state, color, castle_rights)

    def update_board(self, board_state, color, castle_rights=None):
        self.white_turn = color == 'w'
    ...
    def move(self, move):
        ...
        start_color = start_piece[0]

        if start_piece[1] == 'k':
            self.castle_rights &= REMOVE_CASTLE_RIGHTS[start_color + 'k'] & REMOVE_CASTLE_RIGHTS[start_color + 'q']
            ...
            if special == CASTLING:
                self.mid_value[start_color] -= MID_VALUES[start_color + 'r'][CASTLE_ROOK_POSITION[end][0]]
                self.end_value[start_color] -= END_VALUES[start_color + 'r'][CASTLE_ROOK_POSITION[end][0]]

                self[CASTLE_ROOK_POSITION[end][1]] = self[CASTLE_ROOK_POSITION[end][0]]
                self[CASTLE_ROOK_POSITION[end][0]] = EmptySquare

                self.mid_value[start_color] += MID_VALUES[start_color + 'r'][CASTLE_ROOK_POSITION[end][1]]
                self.end_value[start_color] += END_VALUES[start_color + 'r'][CASTLE_ROOK_POSITION[end][1]]
        ...
        if special[0] == PROMOTION:
            self[end] = start_color + special[1]
    ...
    @staticmethod
    def from_fen(fen):
        board, color, castle_rights_fen = fen.split()[:3]

        castle_rights = 0
        if 'W' in castle_rights_fen:
            castle_rights |= CASTLE_RIGHTS['wk']
        if 'w' in castle_rights_fen:
            castle_rights |= CASTLE_RIGHTS['wq']
        if 'B' in castle_rights_fen:
            castle_rights |= CASTLE_RIGHTS['bk']
        if 'b' in castle_rights_fen:
            castle_rights |= CASTLE_RIGHTS['bq']

        return Board(BoardState.from_fen(fen), color, castle_rights)


class Engine:
    def __init__(self, color, time_per_move):
        self.name = 'OutOfStockFish'

        self.color = 'w' if color == PlayerColour.White else 'b'
        self.time_per_move = time_per_move
        self.board = Board()

        self.alpha, self.beta = -INF, INF
        self.pv_len = self.pv_table = self.history_table = None
    ...
    def get_best_move_depth(self, cur_depth, best_move=None):
        def quiescence(alpha, beta, ply):
            ...
            for move in legal_moves:
                ...
                evaluation = -quiescence(-beta, -alpha, ply + 1)
                self.board.unmove()
    ...
    def get_move(self, board_state):
        self.board.update_board(board_state, self.color)

        time_start = time()
        self.time_end = time_start + self.time_per_move * 0.85
        self.timeout = False
...
if __name__ == '__main__':
    engine1 = Engine(PlayerColour.White, 1)
    engine2 = Engine(PlayerColour.Black, 1)

    board = BoardState.from_fen('rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR')
    ...
    while True:
        if turn == 0:
            ...
            board = engine1.board.to_board_state()

            print(board)
            print(-engine1.board.evaluate())

            if engine1.board.is_over():
                print('Checkmate, white wins')
                break

        else:
            move = engine2.get_move(board)
            board = engine2.board.to_board_state()

            print(board)
            print(engine2.board.evaluate())

            if engine2.board.is_over():
